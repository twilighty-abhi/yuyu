...
async function context(data: z.infer<typeof target>, userId: string) {
  ...
  if (!org) return null;
  const allowed = await canAccessEvent({ userId, organisationId: org.id, eventId: data.eventId, eventSeriesId: data.eventSeriesId, permission: EventPermission.PUBLISH_AND_SCHEDULE });
  return allowed ? org : null;
}
function paths(orgSlug: string, data: z.infer<typeof target>) { revalidatePath(`/dashboard/${orgSlug}/${data.eventId ? `event/${data.eventId}` : `series/${data.eventSeriesId}`}`); if (data.eventId) revalidatePath(`/${orgSlug}`); }
...
export async function saveScheduleItem(input: unknown): Promise<ActionResult> {
  ...
  if (data.itemId) {
    const updated = await prisma.eventScheduleItem.updateMany({ where: { id: data.itemId, ...(data.eventId ? { eventId: data.eventId } : { eventSeriesId: data.eventSeriesId }) }, data: { title: data.title, description: data.description, startDateTime: data.startDateTime, endDateTime: data.endDateTime } });
    if (!updated.count) return { ok: false, error: "Schedule item not found." };
  } else {
    const max = await prisma.eventScheduleItem.aggregate({ where: data.eventId ? { eventId: data.eventId } : { eventSeriesId: data.eventSeriesId }, _max: { sortOrder: true } });
    await prisma.eventScheduleItem.create({ data: { ...(data.eventId ? { eventId: data.eventId } : { eventSeriesId: data.eventSeriesId }), title: data.title, description: data.description, startDateTime: data.startDateTime, endDateTime: data.endDateTime, sortOrder: (max._max.sortOrder ?? 0) + 1 } });
  }
...
export async function setScheduleDelay(input: unknown): Promise<ActionResult> {
  ...
  const org = await context(parsed.data, session.user.id); if (!org) return { ok: false, error: "You do not have permission to manage the schedule." };
  const item = await prisma.eventScheduleItem.updateMany({ where: { id: parsed.data.itemId, ...(parsed.data.eventId ? { eventId: parsed.data.eventId } : { eventSeriesId: parsed.data.eventSeriesId }) }, data: { delayMinutes: parsed.data.delayMinutes } });
