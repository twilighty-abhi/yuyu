...
const bodySchema = z.object({
  organisationSlug: z.string().trim().min(1), eventSlug: z.string().trim().min(1),
  action: z.enum(["unlock", "bootstrap", "previewToken", "previewRsvp", "checkInRsvp", "lookup", "undo"]),
  pin: z.string().trim().regex(/^\d{8}$/).optional(), rawInput: z.string().trim().min(1).optional(),
  rsvpId: z.string().trim().min(1).optional(), query: z.string().trim().min(1).max(200).optional(), force: z.boolean().optional(),
});
...
export async function POST(request: Request) {
  ...
  const event = await prisma.event.findFirst({ where: { slug: input.eventSlug, organisation: { slug: input.organisationSlug } }, select: { id: true, slug: true, organisationId: true, endDateTime: true, checkInStationPinHash: true, checkInStationSecretVersion: true, organisation: { select: { slug: true, name: true, logoUrl: true } } } });
  ...
  if (input.action === "unlock") {
    ...
    const proof = createCheckInStationProof(event.id, event.checkInStationSecretVersion, event.endDateTime);
  ...
  if (!hasValidCheckInStationProof(proof, event.id, event.checkInStationSecretVersion, event.endDateTime)) return failure("Station access has expired.", 401);
  ...
  if (input.action === "undo") {
    await prisma.$transaction(async (tx) => { await tx.rSVP.update({ where: { id: rsvp.id }, data: { checkedInAt: null } }); await tx.checkInEvent.create({ data: { rsvpId: rsvp.id, action: "CHECK_IN_UNDONE", source: "venue-station" } }); });
  ...
  if (!rsvp.checkedInAt) {
    const now = new Date();
    const updated = await prisma.$transaction(async (tx) => { const count = await tx.rSVP.updateMany({ where: { id: rsvp.id, checkedInAt: null }, data: { checkedInAt: now } }); if (!count.count) return false; await tx.checkInEvent.create({ data: { rsvpId: rsvp.id, action: "CHECKED_IN", source: "venue-station", occurredAt: now } }); return true; });
    if (updated) { data.checkedInAt = now.toISOString(); data.alreadyCheckedIn = false; }
    else { const current = await prisma.rSVP.findUnique({ where: { id: rsvp.id }, select: { checkedInAt: true } }); data.checkedInAt = current?.checkedInAt?.toISOString() ?? now.toISOString(); data.alreadyCheckedIn = true; }
  }
  revalidatePath(`/${event.organisation.slug}/${event.slug}/check-in`);
