...
import { apiResourceIdSchema, participantCollectionQuerySchema, requestsParticipantAttendance } from "@/lib/api/v1/schemas";
...
export const GET = withApiMonitoring(
  ...
  async (request: Request, routeContext: Context) =>
    handleMachineApiRequest(request, "participants:read", async (context) => {
      ...
      if (requestsParticipantAttendance(query.data) && !context.scopes.has("participants:attendance:read")) {
