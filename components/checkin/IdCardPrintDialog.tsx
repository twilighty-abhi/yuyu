...
export function IdCardPrintDialog(props: {
  ...
  const previewText = settings.template === "bold" ? "#FFFFFF" : "text.primary";
        ...
        <Stack direction={{ xs: "column", md: "row" }} spacing={3}>
          <Stack spacing={2} sx={{ flex: 1, minWidth: 0 }}>
            ...
            <Stack direction="row" spacing={1.5} sx={{ alignItems: "center" }}>
              <TextField label="Accent colour" type="color" value={settings.accentColor} onChange={(event) => update({ accentColor: event.target.value })} slotProps={{ htmlInput: { "aria-label": "Accent colour" } }} sx={{ width: 150 }} />
              <Typography variant="body2" color="text.secondary">Used for the heading and template accents.</Typography>
            </Stack>
          ...
          <Stack spacing={1} sx={{ flex: 1, minWidth: 0, alignItems: "center" }}>
            ...
            <Box sx={{ width: "min(100%, 340px)", aspectRatio: ratio, border: settings.template === "bold" ? 0 : "2px solid", borderColor: "text.primary", bgcolor: previewBg, color: previewText, p: 2.5, display: "flex", flexDirection: "column", boxShadow: 3, overflow: "hidden" }}>
              <Stack direction="row" sx={{ justifyContent: "space-between", alignItems: "flex-start" }}>
                <Box>
                  <Typography sx={{ fontSize: "0.62rem", letterSpacing: "0.14em", fontWeight: 800, color: settings.template === "bold" ? "inherit" : settings.accentColor }}>{settings.badgeLabel}</Typography>
