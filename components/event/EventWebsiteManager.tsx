...
export function EventWebsiteManager(props: Props) {
  ...
  useEffect(() => {
    const markPageDirty = (event: Event) => {
      ...
      if (form?.dataset.eventPageForm === "true") { setDirty(true); setSaved(false); }
    };
  ...
  const save = (action: SaveAction, input: unknown, clearDirty = false) => startTransition(async () => { const result = await action(input); if (!result.ok) { setSaved(false); setError(result.error || "Could not save your changes."); } else { setError(""); setSaved(true); if (clearDirty) setDirty(false); router.refresh(); } });
  const moveSection = (index: number, direction: -1 | 1) => { setDirty(true); setSaved(false); setSections((current) => { const nextIndex = index + direction; if (nextIndex < 0 || nextIndex >= current.length) return current; const next = [...current]; [next[index], next[nextIndex]] = [next[nextIndex], next[index]]; return next.map((section, sortOrder) => ({ ...section, sortOrder })); }); };
