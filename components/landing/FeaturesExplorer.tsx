...
export function FeaturesExplorer() {
  ...
  return (
    <Box sx={{ pb: { xs: 6, md: 10 } }}>
      <Paper variant="outlined" sx={{ position: "relative", overflow: "hidden", p: { xs: 3, sm: 5, md: 7 }, borderRadius: 5, background: "linear-gradient(135deg, rgba(10,132,255,0.14), transparent 48%, rgba(124,245,182,0.12))" }}>
        <Box aria-hidden sx={{ position: "absolute", width: 340, height: 340, right: -140, top: -180, borderRadius: "50%", bgcolor: "primary.main", opacity: 0.1, filter: "blur(10px)" }} />
        <Stack spacing={2.5} sx={{ position: "relative", alignItems: { xs: "flex-start", md: "center" }, textAlign: { xs: "left", md: "center" } }}>
          <Chip label="YUYU FEATURE GUIDE" color="primary" variant="outlined" sx={{ fontWeight: 700, letterSpacing: "0.08em" }} />
          <Typography component="h1" variant="h2" sx={{ maxWidth: 760, fontWeight: 800, letterSpacing: "-0.05em" }}>Everything you need to run great free events</Typography>
      ...
      <Stack spacing={2.5} sx={{ mt: { xs: 5, md: 8 }, mb: 4 }}>
        <Stack direction={{ xs: "column", sm: "row" }} spacing={2} sx={{ justifyContent: "space-between", alignItems: { sm: "flex-end" } }}>
          <Stack spacing={0.75}>
            <Typography variant="overline" color="primary" sx={{ fontWeight: 700, letterSpacing: "0.1em" }}>THE COMPLETE TOOLKIT</Typography>
            <Typography variant="h4" sx={{ fontWeight: 800, letterSpacing: "-0.04em" }}>Every capability, clearly laid out</Typography>
            ...
            return (
              <Grid key={group.id} size={{ xs: 12, md: 6 }}>
                <motion.div initial={prefersReducedMotion ? false : { opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: prefersReducedMotion ? 0 : 0.28, delay: prefersReducedMotion ? 0 : index * 0.035 }}>
                  <Paper variant="outlined" sx={{ height: "100%", p: { xs: 2.5, sm: 3.5 }, borderRadius: 4, transition: "transform 0.2s ease, border-color 0.2s ease, box-shadow 0.2s ease", "&:hover": { transform: "translateY(-3px)", borderColor: "primary.main", boxShadow: "0 14px 36px rgba(10,132,255,0.1)" } }}>
                    <Stack spacing={2}>
                      <Stack direction="row" spacing={1.5} sx={{ alignItems: "flex-start" }}>
                        <Box sx={{ flex: "0 0 auto", width: 46, height: 46, display: "grid", placeItems: "center", borderRadius: 2.5, bgcolor: "rgba(10,132,255,0.1)", color: "primary.main" }}><Icon /></Box>
                        <Box sx={{ minWidth: 0 }}>
      ...
      <Paper variant="outlined" sx={{ mt: { xs: 5, md: 8 }, p: { xs: 3, sm: 5 }, borderRadius: 4, textAlign: "center", background: "linear-gradient(135deg, rgba(124,245,182,0.1), rgba(185,174,255,0.1))" }}>
        <Stack spacing={2} sx={{ alignItems: "center" }}>
          <SettingsSuggestOutlinedIcon color="primary" sx={{ fontSize: 34 }} />
          <Typography variant="h4" sx={{ fontWeight: 800 }}>Ready to make your next event easier?</Typography>
