...
- In-venue attendee ID-card preview and browser printing after a QR, manual, or lookup check-in; A6 portrait is the default, with laser black-and-white and thermal printer profiles, a 4 × 6 thermal-stock preset, organisation branding, card text, and selectable participant registration fields (including organisation and role) with custom printed labels.
