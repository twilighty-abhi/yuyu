-- Atomic, PII-free fallback records for security-sensitive mutations.
...
BEGIN
  FOREACH table_name IN ARRAY ARRAY['Organisation', 'Membership', 'Event', 'EventSeries', 'OrganisationInvite', 'EventInvite', 'SeriesInvite', 'RSVP'] LOOP
    EXECUTE format('CREATE TRIGGER %I BEFORE INSERT OR UPDATE OR DELETE ON %I FOR EACH ROW EXECUTE FUNCTION "write_security_audit_fallback"()', 'audit_fallback_' || lower(table_name), table_name);
  END LOOP;
END;
$$;
