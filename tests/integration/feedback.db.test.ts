...
describe.sequential("event feedback integration", () => {
  beforeAll(async () => {
    ...
    await prisma.eventFeedbackField.create({ data: { formId: anonymousForm.id, key: "comment", label: "Comment", type: "TEXTAREA", required: true } });
  });
  ...
  it("accepts repeat anonymous feedback without collecting an RSVP identity", async () => {
    await expect(submitFeedback({ orgSlug, eventSlug: anonymousEventSlug, answers: { comment: "First" } })).resolves.toEqual({ ok: true, data: { certificateToken: null } });
